...
class DockerClient(dockerHost: Option[String] = None,
  ...
  def run(image: String, args: Seq[String] = Seq.empty[String])(
    ...
    Future {
      ...
      // val start_system_ns = System.nanoTime
      ...
      runCmd(Seq("run", "-d") ++ args ++ Seq(image), config.timeouts.run)
        .andThen {
          ...
          case _ => runSemaphore.release()
            ...
            // log.info(this, s"docker create time = ${(end_system_ns - start_system_ns)/1000000.0} ms")
        }
  ...
  // yanqi, add update to change cpu limit at real time
...
trait DockerApi {
  ...
  /**
   * Update the container with the given id, yanqi
   *
   * @param id the id of the container to remove
