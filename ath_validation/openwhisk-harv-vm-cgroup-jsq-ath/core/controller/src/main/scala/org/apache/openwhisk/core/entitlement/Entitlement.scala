...
// yanqi, for now just replace
// import org.apache.openwhisk.core.loadBalancer.{LoadBalancer, ShardingContainerPoolBalancer}
import org.apache.openwhisk.core.loadBalancer.{LoadBalancer, HarvestVMContainerPoolBalancer}
...
protected[core] abstract class EntitlementProvider(
  ...
  private val activationThrottleCalculator = loadBalancer match {
    ...
    // yanqi, for now replace ShardingContainerPoolBalancer with HarvestVMContainerPoolBalancer
    // later we might need to change the way limits are computed
    case _: HarvestVMContainerPoolBalancer => calculateIndividualLimit _
